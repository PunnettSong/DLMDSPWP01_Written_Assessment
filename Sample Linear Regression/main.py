...
df_ideal = pd.read_csv('ideal.csv').to_numpy()

#Set X training values

X_train = np.array(df_train[:,0]).reshape(-1,df_train.shape[0])
...
y_train = np.array(df_train[:,0]).reshape(-1,df_train.shape[0])
...
print("Coefficient of determination: %.2f" % r2_score(diabetes_y_test, diabaetes_y_pred))
